...
(function () {
  ...
  if (
    window.location.href.includes("digikala.com/product/") ||
        ...
        productContainerSelectors.forEach((selector) => {
          try {
            ...
            containers.forEach((container, index) => {
              ...
              if (
                detectedElements.has(container) ||
                container.offsetHeight < 50
              )
                return;
              detectedElements.add(container);

              const product = extractProductFromContainer(
                container,
  ...
  function isCurrentPageProductPage() {
    ...
    const isProductUrl = productPagePatterns.some((pattern) =>
    ...
    const hasAddToCartButton = document.querySelector(
      '[class*="cart"], [class*="buy"], button[class*="add"]'
    );
    const hasProductPrice = document.querySelector('[class*="price"]');
    const hasProductGallery = document.querySelector(
      '[class*="gallery"], [class*="image-main"]'
    );

    const hasProductPageElements =
      [hasMainProductTitle, hasAddToCartButton, hasProductPrice].filter(Boolean)
        .length >= 2;

    // Few product links suggests single product page, many suggests listing
    const productLinksCount = document.querySelectorAll(
      ...
      try {
        ...
        if (element) {
          ...
          if (text && text.length > 5 && text.length < 500) {
            ...
            if (
              text.match(/[a-zA-Z\u0600-\u06FF]/) &&
              !text.match(/^(قیمت|خرید|تومان|ریال|cart|buy)$/i)
            ) {
              return text;
            }
          }
    ...
    for (const selector of mainImageSelectors) {
      try {
        ...
        if (element && element.src) {
          ...
          if (
            (element.naturalWidth > 200 && element.naturalHeight > 200) ||
      ...
      try {
        ...
        sections.forEach((section) => {
          const products = section.querySelectorAll(
            'a[href*="/product"], a[href*="/p/"], [class*="product"]'
          );
          products.forEach((element, index) => {
            ...
            const container =
              element.closest('[class*="product"], [class*="item"]') || element;
            const product = extractProductFromContainer(
              container,
    ...
    for (const selector of nameSelectors) {
      try {
        ...
        for (const element of elements) {
          if (element) {
            let text =
              element.textContent || element.title || element.alt || "";
            text = text.trim();
            ...
            const isProductText =
              !skipTexts.some((skip) =>
                text.toLowerCase().includes(skip.toLowerCase())
              ) &&
              text.length > 5 &&
              text.length < 300 &&
  ...
  // Listen for messages from popup (Chrome compatible)
  const runtimeAPI = chrome;
