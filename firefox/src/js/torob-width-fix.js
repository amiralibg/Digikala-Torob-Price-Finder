...
(function() {
  ...
  function isOpenedFromExtension() {
    ...
    const hasExtensionIndicator = window.location.href.includes('from-extension') || 
                                  sessionStorage.getItem('opened-from-extension') ||
                                  localStorage.getItem('torob-width-fix-applied');
    
    return hasExtensionIndicator || referrer.includes('moz-extension://') || referrer.includes('chrome-extension://');
  }
  ...
  function applyFullWidthStyling() {
    ...
    console.log('Torob width fix applied: body and containers set to 100% width');
  }
  ...
  // Listen for messages from the extension popup (Firefox compatible)
